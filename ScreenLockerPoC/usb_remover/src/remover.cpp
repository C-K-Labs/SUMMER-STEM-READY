...
// [REFACTOR] Use an anonymous namespace for internal helper functions.
namespace {
...
bool Remover::RemoveScreenLockerFiles() {
    ...
    // 3. Delete log folders (temporarily disabled)
...
// [REFACTOR] This function now uses helper functions to build the batch script.
...
bool Remover::DeleteUnlockHtmlFile() {
    ...
    if (!targetExecutablePath.empty()) {
        // Find the HTML file in the same directory as the executable
        ...
        htmlPath = targetDir + "\\" + Constants::UnlockHtmlFile;
        ...
        htmlPath = exeDir + "\\" + Constants::UnlockHtmlFile;
