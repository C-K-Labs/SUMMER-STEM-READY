        ...
        case WM_LBUTTONDOWN: {
            ...
            if (xPos >= buttonRect.left && xPos <= buttonRect.right && yPos >= buttonRect.top && yPos <= buttonRect.bottom) {
                ...
                if (pThis && !pThis->browserRunning) { // Prevent duplicate execution if browser is already running
                    ...
                    // Call the unlock process in a new thread
...
void ScreenLocker::OpenControlledBrowser() {
    ...
    do {
        ...
        std::string htmlFile = exeDir + "\\" + Constants::UnlockHtmlFile;
...
// [REFACTOR] Main function to handle the unlock process, now orchestrating helper functions.
void ScreenLocker::HandleUnlockProcess() {
    ...
    if (!browserHwnd) {
        SaveToLogFile(userID, "BROWSER_CREATION_FAILED");
        MessageBoxW(hWnd, L"Failed to open the unlock interface.\nPlease check your system configuration and try again.", L"Error", MB_OK | MB_ICONERROR);
        RestoreScreenLockerTopmost();
        browserRunning = false; // Reset browser state
        return;
    }

    // 2. Wait for a signal from the browser.
    bool signalReceived = false;
    bool browserWasClosed = false;
    WaitForBrowserSignal(signalReceived, browserWasClosed);

    // 3. The browser is now closed, so release the cursor confinement.
    ReleaseCursorConfinement();

    // 4. Process the outcome.
    ProcessUnlockResult(signalReceived, browserWasClosed);

    // 5. Reset state for the next potential unlock attempt.
    browserRunning = false;
    browserHwnd = nullptr;
