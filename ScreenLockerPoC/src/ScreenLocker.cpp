...
#include <vector>
#include <tlhelp32.h>
#include <dbt.h> // [NEW FEATURE] For device broadcast types
...
ScreenLocker::~ScreenLocker() {
    ...
    // [FIX] Wait for all worker threads to complete before destruction
    SaveToLogFile(userID, "WAITING_FOR_WORKER_THREADS_TO_FINISH");
    {
    ...
    SaveToLogFile(userID, "ALL_WORKER_THREADS_COMPLETED");
...
void ScreenLocker::EnableAdvancedSecurity() {
    ...
    // [FIX] Disable shell hooking because the DLL implementation is not available
...
bool ScreenLocker::BlockSystemKeys(DWORD vkCode, bool isSystemKey) {
    // Block system shutdown related key combinations
...
LRESULT CALLBACK ScreenLocker::SecondaryWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (uMsg) {
        ...
        // [NEW FEATURE] Handle USB device insertion/removal
    ...
    switch (uMsg) {
        ...
        case WM_LBUTTONDOWN: {
            ...
            if (xPos >= buttonRect.left && xPos <= buttonRect.right && yPos >= buttonRect.top && yPos <= buttonRect.bottom) {
                ...
                if (pThis && !pThis->browserRunning) { // Prevent duplicate execution if browser is already running
                    pThis->SaveToLogFile(pThis->userID, "UNLOCK_BUTTON_CLICKED");
                    
                    // [FIX] Create and manage the thread instead of detaching
                    {
                        std::lock_guard<std::mutex> lock(pThis->threadMutex);
                        pThis->workerThreads.emplace_back(&ScreenLocker::HandleUnlockProcess, pThis);
                    }
                }
...
void ScreenLocker::OpenControlledBrowser() {
    ...
    // [FIX] COM initialization is now handled by the calling thread (HandleUnlockProcess).
    ...
    do {
        // [FIX] Create the temporary HTML file in the user's %TEMP% directory.
        // This is the correct and reliable location, avoiding permission issues.
        char tempPathBuffer[MAX_PATH];
        GetTempPathA(MAX_PATH, tempPathBuffer); // Gets C:\Users\user\AppData\Local\Temp

        char tempFileName[MAX_PATH];
        // Creates a unique temporary file name with a .html extension
        // Note: The function creates the file, so we use its name.
        GetTempFileNameA(tempPathBuffer, "unlock", 0, tempFileName);
        
        // The actual path to the temp file
        std::string htmlFile = tempFileName;
        std::string htmlContent = GenerateHTMLContent();  // UserID is already inserted
        ...
        // 3 seconds after automatic allow (for security)
    ...
    // [FIX] COM uninitialization is now handled by the calling thread.
    // if (SUCCEEDED(hrInit)) {
    //     CoUninitialize();
    // }

    // Browser state is maintained
...
// [FIX] This function now correctly handles COM initialization for a worker thread.
void ScreenLocker::HandleUnlockProcess() {
    // Reset COM library
    HRESULT hrInit = CoInitializeEx(NULL, COINIT_MULTITHREADED);
    if (FAILED(hrInit)) {
        SaveToLogFile(userID, "UNLOCK_THREAD_COM_INIT_FAILED: " + std::to_string(hrInit));
        return;
    }

    SaveToLogFile(userID, "UNLOCK_PROCESS_STARTED");

    // [FIX] Pause security timer before creating browser to prevent race condition
    if (securityCheckTimer) {
        KillTimer(hWnd, Constants::SecurityTimerID);
        SaveToLogFile(userID, "SECURITY_CHECK_TIMER_PAUSED");
    }

    // 1. Open browser
    OpenControlledBrowser();

    // [FIX] Resume security timer after browser creation
    if (advancedSecurityEnabled) {
        securityCheckTimer = SetTimer(hWnd, Constants::SecurityTimerID, 1000, SecurityCheckCallback);
        SaveToLogFile(userID, "SECURITY_CHECK_TIMER_RESUMED");
    }
    
    // If browser handle is not obtained, handle failure
    if (!browserHwnd) {
        SaveToLogFile(userID, "BROWSER_CREATION_FAILED_IN_UNLOCK_PROCESS");
        MessageBoxW(hWnd, L"Failed to open the unlock interface.\nPlease try again.", L"Error", MB_OK | MB_ICONERROR);
        RestoreScreenLockerTopmost();
        browserRunning = false; 
    } else {
        // 2. Wait for browser signal
        bool signalReceived = false;
        bool browserWasClosed = false;
        WaitForBrowserSignal(signalReceived, browserWasClosed);

        // 3. Release cursor confinement
        ReleaseCursorConfinement();

        // 4. Process result
        ProcessUnlockResult(signalReceived, browserWasClosed);
    ...
    // Uninitialize COM library before thread exit
    CoUninitialize();
}
