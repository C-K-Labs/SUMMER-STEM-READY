...
void ScreenLocker::OpenControlledBrowser() {
   ...
   browserRunning = true;
   SaveToLogFile(userID, "STARTING_BROWSER_CREATION_PROCESS");

   IWebBrowser2* pWebBrowser = NULL;
   BSTR url = NULL;

   do {
       SaveToLogFile(userID, "CREATING_HTML_FILE");
       char tempPathBuffer[MAX_PATH];
       GetTempPathA(MAX_PATH, tempPathBuffer); // Gets C:\Users\user\AppData\Local\Temp

       // Create unique HTML filename with proper extension
       SYSTEMTIME st;
       GetLocalTime(&st);
       char uniqueFileName[MAX_PATH];
       sprintf_s(uniqueFileName, "%sunlock_%04d%02d%02d_%02d%02d%02d.html", 
               tempPathBuffer,
               st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);

       std::string htmlFile = uniqueFileName;
       ...
       SaveToLogFile(userID, "CREATING_FILE_URL");
       // Create file:// URL
       ...
       SaveToLogFile(userID, "CREATING_IE_INSTANCE");
       ...
       SaveToLogFile(userID, "IE_INSTANCE_CREATED_SUCCESS");

       SaveToLogFile(userID, "SETTING_BROWSER_PROPERTIES");
       // Browser Settings
       ...
       SaveToLogFile(userID, "STARTING_NAVIGATION");
       ...
       SaveToLogFile(userID, "NAVIGATION_COMPLETED_FINDING_WINDOW");
       ...
       if (browserHwnd) {
           SaveToLogFile(userID, "BROWSER_WINDOW_FOUND_CONTROLLING");
           ControlBrowserWindow();
       } else {
           SaveToLogFile(userID, "BROWSER_WINDOW_NOT_FOUND");
       }
   ...
   SaveToLogFile(userID, "CLEANING_UP_BROWSER_RESOURCES");
    // Clean up resources
    ...
    SaveToLogFile(userID, "EXITING_OPENCONTROLLEDBROWSER_FUNCTION");
}
...
void ScreenLocker::HandleUnlockProcess() {
    ...
    // Execute signal detection logic only when browserHwnd handle is valid
    if (browserHwnd) {
        SaveToLogFile(userID, "WAITING_FOR_BROWSER_SIGNAL");
        bool signal_received = false;
        bool browser_still_exists = true;

        // Wait for signal for about 1 minute (250ms * 120 times)
        for (int i = 0; i < 120 && browser_still_exists; i++) {
            ...
            // Check if browser window became invisible (minimized, etc.)
            if (!IsWindowVisible(browserHwnd)) {
                SaveToLogFile(userID, "BROWSER_WINDOW_HIDDEN");
                // Make window visible again and set as topmost
        ...
        // 2. Proceed with server status check only when signal received from browser
        if (signal_received) {
            ...
            // Branch processing based on actual server status
            if (status.find("unlocked") != std::string::npos) {
                SaveToLogFile(userID, "STATUS_UNLOCKED_BY_SERVER");
                
                // Execute remover with admin privileges if 'unlocked' status
                if (ExecuteRemoverWithAdmin()) {
                    SaveToLogFile(userID, "REMOVER_EXECUTION_INITIATED");
                    // Exit screen locker when remover execution starts successfully
                    PostMessage(hWnd, WM_DESTROY, 0, 0);
                } else {
                    // When execution fails (user clicks "No" in UAC, etc.)
                ...
                // Unknown status or server error (404, etc.)
    ...
    // Reset browser execution state for next request
    browserRunning = false;
    browserHwnd = nullptr;
