...
bool Remover::ExecuteRemoval() {
    ...
    // 1단계: 레지스트리 설정 복원
    if (!RestoreFileExtensionSettings()) {
        LogAction("REGISTRY_RESTORE_FAILED_CONTINUING");
        // 레지스트리 복원 실패해도 계속 진행
    }
...
bool Remover::RestoreFileExtensionSettings() {
    LogAction("RESTORING_FILE_EXTENSION_SETTINGS");
    ...
    if (result == ERROR_SUCCESS) {
        // Set HideFileExt to 0 (show known file extensions - Windows default)
        DWORD showExtensions = 0;
        result = RegSetValueEx(hKey, L"HideFileExt", 0, REG_DWORD, 
                              (BYTE*)&showExtensions, sizeof(DWORD));
        
        if (result == ERROR_SUCCESS) {
            LogAction("FILE_EXTENSION_SETTINGS_RESTORED");
            
            // Refresh Explorer to apply changes immediately
            SHChangeNotify(SHCNE_ASSOCCHANGED, SHCNF_IDLIST, NULL, NULL);
            LogAction("EXPLORER_REFRESHED_FOR_RESTORE");
            
            RegCloseKey(hKey);
            return true;
        } else {
            LogAction("FILE_EXTENSION_RESTORE_FAILED: " + std::to_string(result));
            RegCloseKey(hKey);
            return false;
        }
    } else {
        LogAction("REGISTRY_OPEN_FAILED_FOR_RESTORE: " + std::to_string(result));
        return false;
    }
}